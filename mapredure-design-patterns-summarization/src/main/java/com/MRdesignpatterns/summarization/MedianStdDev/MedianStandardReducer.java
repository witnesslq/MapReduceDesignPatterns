...
public class MedianStandardReducer extends Reducer<Text, SortedMapWritable, Text, DoubleWritable> {

	private DoubleWritable outTuple = new DoubleWritable();
	
	private TreeMap<Long,Long> netWorkFlow = new TreeMap<Long,Long>();
	...
	protected void reduce(Text key,Iterable<SortedMapWritable> values,Context context)
		...
		while(it.hasNext()){
			...
			if(entrySetIterator.hasNext()){
				...
				long flow =   ((LongWritable)entry.getKey()).get();
		...
		for(Entry<Long, Long> entry:netWorkFlow.entrySet()){
